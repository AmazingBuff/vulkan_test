...
public:
    ...
    void configure_uniform_buffer(const std::string& res_name, const std::string& name, uint32_t set, uint32_t binding, uint32_t count, VkDeviceSize size, const std::array<std::unordered_map<uint32_t, VkDescriptorSet>, k_Max_Frames_In_Flight>& descriptor_sets);
...
void VK_CLASS(ResourceManager)::map_vertex_or_index_buffer(const std::string& name, const std::vector<Ty>& vertices_or_indices) const
{
    ...
    if constexpr (std::is_same_v<Ty, Vertex>)
    {
        ...
        m_vertex_buffer->copy_buffer_from(m_staging_buffer, 0, info.offset, buffer_size);
    }
    else
    {
        ...
        m_index_buffer->copy_buffer_from(m_staging_buffer, 0, info.offset, buffer_size);
    }
}
