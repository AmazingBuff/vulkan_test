...
ModelManager::~ModelManager()
{
	...
	{
		for (auto& texture : std::views::values(resource->textures))
		{
			if (texture->data)
	...
	for (auto& dir_entry : std::filesystem::directory_iterator{ MODEL_PATH })
	{
		...
		for (auto& file : std::filesystem::directory_iterator{ dir_entry })
		{
			...
			if (file_name.find(".png") != std::string::npos)
			{
                ...
                texture->data = stbi_load(file.path().generic_string().c_str(), &texture->width, &texture->height,
                ...
                resource->textures.emplace(name, texture);
			}
		}
...
const std::shared_ptr<ModelResource>& ModelManager::get_model_resource(const std::string_view& name)
{
