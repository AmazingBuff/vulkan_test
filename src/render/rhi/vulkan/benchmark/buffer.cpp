...
void VK_CLASS(Buffer)::map_memory(const void* src_data, VkDeviceSize size, VkDeviceSize dst_offset) const
{
    ...
    else
    {
        ...
        staging_buffer->init(size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
...
void VK_CLASS(Buffer)::copy_buffer_from(const std::shared_ptr<VK_CLASS(Buffer)>& src_buffer, VkDeviceSize dst_offset,
...
{
    VK_CLASS(Buffer)::initialize(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
}

void VK_CLASS(UniformBuffer)::update_descriptor_set(
    const std::vector<UniformBufferLayout>&              layouts,
